...
export const InventoryTable: React.FC = () => {
  ...
  const getColWidth = useMemo(
    ...
    [dashboard.getColWidth]
  );
  const handleStartResize = dashboard.handleStartResize ?? (() => {});
  const handleAutoFitColumn = dashboard.handleAutoFitColumn ?? (() => {});
  const resizingCol = dashboard.resizingCol ?? null;
  ...
  const onFrcBodFilterClick = dashboard.onFrcBodFilterClick ?? (() => {});
  ...
  const dragOverCol = dashboard.dragOverCol ?? null;
  const setDragOverCol = dashboard.setDragOverCol ?? (() => {});
  const handleColumnDrop = dashboard.handleColumnDrop ?? (() => {});
  ...
  const columnFilters = dashboard.columnFilters ?? {};
  const setColumnFilters = dashboard.setColumnFilters ?? (() => {});
  const columnOptionsMap = dashboard.columnOptionsMap ?? {};
  const frcBodValues = dashboard.frcBodValues ?? [];
  const frcBodCounts = dashboard.frcBodCounts ?? {};
  const frcBodCol = dashboard.frcBodCol ?? '';
  const virtualRows = dashboard.virtualRows ?? [];
  const paginatedDisplayRows = dashboard.paginatedDisplayRows ?? [];
  const paddingTop = dashboard.paddingTop ?? 0;
  const paddingBottom = dashboard.paddingBottom ?? 0;
  const groupByColumn = dashboard.groupByColumn;
  const onSelectGroupRows = dashboard.onSelectGroupRows;
  const toggleGroupCollapse = dashboard.toggleGroupCollapse;
  const measureElementRef = dashboard.measureElementRef;
  const sortConfig = dashboard.sortConfig ?? { key: null, direction: null };
  const handleToggleSort = dashboard.handleToggleSort ?? (() => {});
  const tableDensity = dashboard.tableDensity ?? 'compact';
